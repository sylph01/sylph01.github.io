...
LLMが環境に存在する時代のテックトークでは、（少なくとも経験者は）LLMが教えてくれる程度のことでトークをしてはいけないし、それが暗黙の評価基準に入っていると言っても過言ではない、と考える。その上で私はそうでないトークをしているつもりである。（一方でそれなので自動的に価値が高い、という主張をしているのではない。価値は別のところにも現れてその部分でも評価される）
